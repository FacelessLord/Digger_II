...
namespace Digger.Objects
{
	public class Monster : Living
	{
		...
		public override CreatureCommand Update(int x, int y)
		{
			...
			if (Game._time % 3 == 0)
			{
				if ((Game._map[x + 1, y] == null || !Game._map[x + 1, y].IsSolidObject()) && _way == "left")
				{
					moving._deltaX++;
					_way = "left";
				...
				if ((Game._map[x - 1, y] == null || !Game._map[x - 1, y].IsSolidObject()) && _way == "right")
				{
					moving._deltaX--;
					_way = "right";
				}
				...
				if
				(Game._map[x + moving._deltaX, y] != null
				 && (Game._map[x + moving._deltaX, y + moving._deltaY].IsSolidObject()))
				{
					moving._deltaX = 0;
				}
